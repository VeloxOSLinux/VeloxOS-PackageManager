...
class PackageListWidget(QWidget):
    def __init__(self, parent=None):
        ...
        # Spalten-Verhalten: Name dehnt sich aus, Quelle nur so viel wie nötig
        ...
        # Status-Label am unteren Rand
        self.count_label = QLabel("0 Pakete", self)
        layout.addWidget(self.count_label)

    def set_packages(self, packages):
        """Wird aufgerufen, wenn neue Daten aus der DB geladen wurden."""
        self.all_packages = packages
        self.refresh_list()

    def format_display_name(self, pkg_name):
        """Konvertiert technische Namen (org.mozilla.firefox) in lesbare Titel (firefox)."""
        if not pkg_name:
            return "Unbekannt"
        if "." in pkg_name:
    ...
    def get_icon(self, pkg):
        """Versucht das passende Icon für ein Paket zu finden."""
        icon_data = pkg.get("icon_url", "") or ""
        # FIX: Hier nutzen wir jetzt default.png statt logo.png
        fallback_icon_path = os.path.join(self.icons_path, "default.png")

        # 1. System-Theme (Wird durch veloxos.py mit theme:// markiert)
        if icon_data.startswith("theme://"):
            theme_name = icon_data.replace("theme://", "")
            icon = QIcon.fromTheme(theme_name)
            if not icon.isNull():
                return icon
            return QIcon(fallback_icon_path)

        # 2. Direkter Dateipfad (Tiefensuche aus veloxos.py)
        if icon_data.startswith("/"):
            if os.path.exists(icon_data):
                return QIcon(icon_data)

        # 3. Flathub-Cache oder lokaler Cache
        pkg_name = pkg.get("name", "") or "unknown"
        clean_name = "".join([c for c in pkg_name if c.isalnum()]).lower()
        cache_file = os.path.join(self.cache_path, f"{clean_name}.png")
        if os.path.exists(cache_file):
            return QIcon(cache_file)

        # 4. Wenn gar nichts hilft: Das Default-Icon anzeigen
        if os.path.exists(fallback_icon_path):
            return QIcon(fallback_icon_path)

        # Letzter Ausweg, falls selbst default.png fehlt (leeres Icon statt Absturz)
        return QIcon()

    def refresh_list(self):
        """Aktualisiert die Anzeige basierend auf Suche und Filtern."""
        self.list.clear()
